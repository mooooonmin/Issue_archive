...
# ====== [Slack 설정] 사용자가 직접 URL 입력 ======
# 예: "https://hooks.slack.com/services/XXX/YYY/ZZZ"
SLACK_WEBHOOK_URL = ""  # <-- 본인의 Slack Incoming Webhook URL을 입력하세요.

# requests는 별도 설치 필요할 수 있습니다. (pip install requests)
import requests
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.timer.setInterval(10_000)  # 10초 주기
        ...
        self.price_queries_disabled_today = False  # 당일 시세 조회 중단 플래그
        self.journal_exported_today = False        # 당일 매매일지 CSV 저장 여부
        self.sell_time = time(15, 0, 0)           # <<< 15:00 정각 일괄 매도

        # opt10170용 계좌 비밀번호 (환경에 따라 필요 없을 수 있음)
        self.account_password = ""  # 필요 시 비밀번호 입력
    ...
    def notify_slack(self, text: str):
        """buysell_log에 표시되는 문자열과 동일 텍스트를 슬랙으로 전송."""
        url = (SLACK_WEBHOOK_URL or "").strip()
        if not url or not text:
            return
        try:
            requests.post(url, json={"text": text}, timeout=2)
        except Exception:
            pass
    ...
    def on_start(self):
        ...
        self.running = True
        self.sell_executed_today = False
        self.price_queries_disabled_today = False
        self.journal_exported_today = False
        self.bought.clear()
    ...
    def on_tick(self):
        """10초마다 호출: 시세 갱신, 돌파 체크, 15:00 일괄 매도 및 매매일지 CSV 저장."""
        ...
        if now.time() >= self.sell_time and not self.sell_executed_today:
            ...
            # 매도했든(보유 있었음) 없었든, 15:00 이후에는 조회 중단
            self.sell_executed_today = True
            self.price_queries_disabled_today = True

            # <<< 통합 포인트: 15:00에 매매일지 CSV 저장(하루 1회)
            if not self.journal_exported_today:
                try:
                    out_path = self.export_daily_journal_csv(days=15, out_path="매매일지.csv")
                    msg = f"[매매일지 저장] {out_path}"
                    self.buysell_log.append(msg)
                    self.buysell_log.moveCursor(self.buysell_log.textCursor().End)
                    self.notify_slack(msg)
                except Exception as e:
                    self.buysell_log.append(f"[매매일지 오류] {e}")
                    self.buysell_log.moveCursor(self.buysell_log.textCursor().End)
                finally:
                    self.journal_exported_today = True
    ...
    def append_buysell_log(self, code: str, name: str, price: int, qty: int, side: str):
        """매매 로그: [매수|매도] [종목코드] [종목명] [가격] [수량] -> GUI & Slack"""
        ...
        self.buysell_log.moveCursor(self.buysell_log.textCursor().End)
        # Slack 알림
        self.notify_slack(line)

    # ===========================================
    # <코드2> 통합: opt10170 매매일지 조회/CSV 저장
    # ===========================================
    def fetch_day_journal(self, date_str: str, danju: int = 2, cash_credit: int = 0) -> pd.DataFrame:
        """
        opt10170: 당일매매일지
        - date_str: YYYYMMDD
        - danju: 1(당일매수에 대한 당일매도), 2(당일매도전체)
        - cash_credit: 0(전체), 1(현금만), 2(신용만)
        """
        df = self.kiwoom.block_request(
            "opt10170",
            계좌번호=self.account,
    ...
    def export_daily_journal_csv(self, days: int = 15, out_path: str = "매매일지.csv") -> str:
        """
        오늘 포함 과거 N일(기본 15일)의 당일매매일지 합쳐 CSV 저장.
        - 호출 시 UI 프리즈를 줄이기 위해 TR 사이 QTest.qWait(500) 사용
        """
        ...
        for i in range(days):
            ...
            try:
                df = self.fetch_day_journal(date_str, danju=2, cash_credit=0)
                if df is not None and not df.empty:
                    frames.append(df)
                    self.textboard.append(f"[opt10170] {date_str}: {len(df)} rows")
                else:
                    self.textboard.append(f"[opt10170] {date_str}: no data")
            except Exception as e:
                self.textboard.append(f"[opt10170] {date_str}: error -> {e}")
            self.textboard.moveCursor(self.textboard.textCursor().End)

            QTest.qWait(500)  # TR 사이 0.5초 대기
