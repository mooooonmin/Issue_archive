...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.timer.setInterval(10_000)  # (변경) 10초 주기
        ...
        self.sell_time = time(15, 00, 0) # 15:00 정각 일괄 매도
    ...
    def on_tick(self):
        ...
        if (now.time() >= self.sell_time) and not self.sell_executed_today and self.bought:
            for code in list(self.bought):
                # (허용) 매도 시 현재가 1회 조회하여 로그에 사용
                info = self.fetch_basic_info(code)
                if info is None:
                    # 가격 조회 실패하더라도 시장가 매도 시도
        ...
        for code in self.codes:
            if code in self.bought:
                # (변경) 매수한 종목은 당일 더 이상 현재가 조회하지 않음
                continue
