...
SLACK_WEBHOOK_URL = ""  # <-- 여기에 본인의 Slack Incoming Webhook URL을 입력하세요.
...
class MainWindow(QMainWindow):
    ...
    def notify_slack(self, text: str):
        """
        buysell_log에 표시되는 문자열과 '동일한 텍스트'를 슬랙으로 전송.
        Webhook URL이 비어있으면 조용히 스킵합니다.
        """
        url = (SLACK_WEBHOOK_URL or "").strip()
        if not url or not text:
            return
        try:
            # Slack Incoming Webhook은 {"text": "..."} 형태로 수신
            resp = requests.post(url, json={"text": text}, timeout=2)
            # 200이 아닐 수도 있으니 로깅 원하면 아래 주석 해제
            # if resp.status_code != 200:
            #     self.buysell_log.append(f"[Slack전송오류] status={resp.status_code} body={resp.text[:120]}")
        except Exception as e:
            # 슬랙 오류로 인해 매매 로직이 중단되지 않도록 조용히 무시 (원하면 아래 로그)
            # self.buysell_log.append(f"[Slack전송예외] {e}")
            pass
    ...
    def append_buysell_log(self, code: str, name: str, price: int, qty: int, side: str):
        ...
        - GUI buysell_log에 표시되는 내용과 '동일한 문자열'을 슬랙으로도 전송
        """
